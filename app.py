...
def update_sheet(df_actualizado):
    # Actualiza todo el bloque de datos en la hoja
    sheet.update([df_actualizado.columns.values.tolist()] + df_actualizado.values.tolist())
...
with tab_diario:
    ...
    st.button("🔄 Refrescar Datos", on_click=lambda: st.session_state.update(df=get_data()))
